...
if (localStorage.getItem('Authorization')) {
	(async () => {
		try {
			const {
				data: { favoritePR: id }
			} = await axios.get('https://mighty-citadel-34235.herokuapp.com/user/me', { headers: { Authorization: 'Bearer ' + localStorage.getItem('Authorization') } });
			favoritePR = id;
		} catch (err) {
...
document.getElementById('searchPointRelais').addEventListener('submit', async e => {
	...
	try {
		const { data } = await axios.post('https://mighty-citadel-34235.herokuapp.com/mr/search', body);
		console.log(data);
